...
public class JsonUtils {
    ...
    private enum Singleton {
        ...
        private JsonUtils singleton;

        //JVM会保证此方法绝对只调用一次
        Singleton() {
            singleton = new JsonUtils(Include.NON_DEFAULT).enableSimple();
        }
     ...
     * @return Map
     */
    public <K, V> Map<K, V> fromJsonToMap(String jsonString, Class<K> keyClass, Class<V> valueClass) {
        if (StringUtils.isEmpty(jsonString)) {
            return null;
        }

        try {
            JavaType javaType = createCollectionType(ArrayList.class, keyClass, valueClass);
            return mapper.readValue(jsonString, javaType);
        } catch (IOException e) {
            logger.warn("parse json string error:" + jsonString, e);
            return null;
