...
public class ObjectUtils {
     ...
     * @return Map对象
     */
    public static Map<String, Object> objectToMap(Object obj, List<String> includeFields, List<String> sensitiveKey) {
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            if (includeFields.contains(fieldName)) {
                try {
                    ...
                    if (sensitiveKey.contains(fieldName)) {
                        value = value.toString().replaceAll("(\\d{3})\\d{4}(\\d{4})", "$1****$2");
                    }
